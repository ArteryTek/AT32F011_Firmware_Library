...
void usart_init(usart_type* usart_x, uint32_t baud_rate, usart_data_bit_num_type data_bit, usart_stop_bit_num_type stop_bit)
{
  ...
  if(usart_x == USART1)
  {
    ...
    else if(CRM_USART_CLOCK_SOURCE_HICK48 == usart_clk)
    {
      apb_clock = HICK_VALUE * 6;
    }
  }
  else if(usart_x == USART2)
  {
  ...
  usart_x->ctrl1_bit.dbn0 = data_bit & 0x01;
  usart_x->ctrl1_bit.dbn1 = data_bit >> 1;
...
/**
  ...
  *         - USART_LPWUF_INT: low power wakeup interrupt
  *         - USART_RTOD_INT:  receiver time out detection flag interrupt
  *         - USART_CMD_INT:   character match detection flag interrupt
...
/**
  ...
  *         - USART_OCCUPY_FLAG: receiver occupy flag
  *         - USART_CMDF_FLAG:   character match detection flag
  *         - USART_LPWUF_FLAG:  low power wake up flag
  *         - USART_TXON_FLAG:   receive turned on flag
  *         - USART_RXON_FLAG:   transmit turned on flag
...
/**
  ...
  * @param  usart_x: select the usart or the uart peripheral.
  *         this parameter can be one of the following values:
  *         USART1, USART2, USART3, USART4, UART4, UART5, USART6, UART7,or UART8.
...
/**
  ...
  * @param  usart_x: select the usart or the uart peripheral.
  *         this parameter can be one of the following values:
  *         USART1, USART2, USART3, USART4, USART4
  * @param  new_state: new state of the irda mode.
  *         this parameter can be: TRUE or FALSE.
  * @retval none
  */
