...
/** @addtogroup 011_CRM_sclk_switch CRM_sclk_switch
  * @{
  */

static void clkout_config(void);
static void sclk_64m_hick_config(void);
static void sclk_80m_hext_config(void);
...
/**
  * @brief  config sclk 80 mhz with hext clock source.
  * @note   the system clock is configured as follow:
  ...
  * @retval none
  */
void sclk_80m_hext_config(void)
{
  /* reset crm */
  crm_reset();
