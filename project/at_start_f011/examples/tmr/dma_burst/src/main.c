...
/** @addtogroup 011_TMR_dma_burst TMR_dma_burst
  * @{
  */
...
int main(void)
{
  ...
  /* tmr1 configuration: generate 1 pwm signal using the dma burst mode:
     ...
     system_core_clock is set to 80 mhz .
     the tmr1 period is 4.88 khz: tmr1 frequency = tmr1 counter clock/(pr + 1)
                                               = 20 mhz / 4096 = 4.88khz
     tmr1 channel1 duty cycle = (tim1_c1dt/ tmr1_pr)* 100 = 33.33% */

  tmr_base_init(TMR1, 0xFFFF, (crm_clocks_freq_struct.sclk_freq/20000000)-1);
