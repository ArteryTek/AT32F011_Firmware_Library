...
/** @addtogroup 011_TMR_cascade_synchro TMR_cascade_synchro
  * @{
  */
...
int main(void)
{
   ...
   *  the timxclk is fixed to 80 mhz, the tmr1 counter clock is 80 mhz.
    ...
    (tmr3 frequency)/ (tmr3 period + 1) = 100 khz and a duty cycle equal
    to tmr3_c1dt/(tmr3_pr + 1) = 50%

    the tmr15 is running at:
    (tmr15 frequency)/ (tmr15 period + 1) = 10 khz and a duty cycle equal
    to tmr15_c1dt/(tmr15_pr + 1) = 50%
  */

  tmr_base_init(TMR1, 79, 0);
  tmr_cnt_dir_set(TMR1, TMR_COUNT_UP);

  tmr_base_init(TMR3, 9, 0);
  tmr_cnt_dir_set(TMR3, TMR_COUNT_UP);

  tmr_base_init(TMR15, 9, 0);
  ...
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_1, 48);
  ...
  tmr_channel_value_set(TMR15, TMR_SELECT_CHANNEL_1, 5);
