...
/** @addtogroup 011_USART_modbus_character_match USART_modbus_character_match
  * @{
  */
...
uint8_t text_info[] = "usart example: modbus_character_match\r\n";
...
void usart_configuration(void)
{
  ...
  usart_init(USART1, 9600, USART_DATA_7BITS, USART_STOP_2_BIT);
  usart_parity_selection_config(USART1, USART_PARITY_NONE);
  usart_transmitter_enable(USART1, TRUE);
  usart_receiver_enable(USART1, TRUE);

  usart_wakeup_id_set(USART1, '\n');
  
  nvic_irq_enable(USART1_IRQn, 0, 0);
  usart_interrupt_enable(USART1, USART_CMD_INT, TRUE);
